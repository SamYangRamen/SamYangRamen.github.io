...
약 15분 간 전체적으로 30초마다 도형 하나씩, 더 자세히 말하자면 각 계정별로 1분마다 도형 하나씩 만든 결과

![image](https://user-images.githubusercontent.com/53200166/135215874-91825cc3-49ae-424c-9101-d96ba3b364fc.png)

![image](https://user-images.githubusercontent.com/53200166/135215904-942903a1-e3e7-48cb-aa60-6b887ffa8832.png)

위와 같이 10분 간격으로 묶여서 버전 기록이 툴페인에 생성되는 것을 볼 수 있으며, 각 계정별 편집 내역은 확인할 수 없었음
...
- 일정 시간 동안(약 몇 초로 예상)  편집이 없을 경우, 버전 기록이 다시 시작하게 된 시점부터 편집을 멈추기 직전까지 편집된 내용이 하나로 묶여서 기록됨
  - 1분 마다 한 번씩 편집된 내역들은 각각 개별적으로 기록되었음
  - 30초 마다 한 번씩 편집된 내역들은 각각 개별적으로 기록되었음
  - 5초 마다 한 번씩 편집된 내역들은 하나로 묶여서 기록되었음
  - 1초 마다 한 번씩 편집된 내역들은 하나로 묶여서 기록되었음
- 다중 편집 시 각각의 오브젝트 단위로 개별 기록 내역이 확인되는데, 테두리 색으로 표현
...
약 25분 간 전체적으로 30초마다 도형 하나씩, 더 자세히 말하자면 각 계정별로 1분마다 도형 하나씩 만든 결과

![image](https://user-images.githubusercontent.com/53200166/135211286-e8241ea6-e6a0-40c6-81cf-7dde9d23daf9.png)

![image](https://user-images.githubusercontent.com/53200166/135211336-fc3fe0f4-dcab-44d8-9a27-45312f1e3973.png)

위와 같이 30초 마다 버전 기록이 하나씩 툴페인에 생성되는 것을 볼 수 있으며, 각 계정별 편집 내역은 도형 단위로 이루어진다는 것을 알 수 있었음

도형 하나에 텍스트 편집만 번갈아가면서 할 경우 각 계정별 편집 내역이 보이지 않았으나, 해당 도형의 최초 생성이 '띵똥' 계정에 의한 것이었다면 계속 하늘색 테두리로 표시되는 것임



#### 테스트 5

도형 하나를 복사하여 1초/5초마다 한 번씩 붙여넣기를 시도한 결과

![image](https://user-images.githubusercontent.com/53200166/135212905-426020b2-eb16-4b3a-942e-4ee70ebfd955.png)

위와 같이 하나의 내역으로 묶여서 기록되는 것을 알 수 있었음
...
약 2분 간 전체적으로 30초마다 표 하나씩, 더 자세히 말하자면 각 계정별로 1분마다 표 하나씩 만든 결과

![image](https://user-images.githubusercontent.com/53200166/135219233-2d2605b7-a3e4-42a3-aaaa-cc0896e37516.png)

위와 같이 모두 하나로 묶여서 버전 기록이 툴페인에 생성되는 것을 볼 수 있으며, (마지막 편집 순서의 편집 내역이 빨간색으로 표시되기는 하지만) 각 계정별 편집 내역은 확인할 수 없었음
...
- 일정 시간 동안(약 몇 초로 예상)  편집이 없을 경우, 버전 기록이 다시 시작하게 된 시점부터 편집을 멈추기 직전까지 편집된 내용이 하나로 묶여서 기록됨
  ...
  - 5초 마다 한 번씩 편집된 내역들은 하나로 묶여서 기록되었음
  - 1초 마다 한 번씩 편집된 내역들은 하나로 묶여서 기록되었음
- 다중 편집 시 텍스트 단위로 개별 기록 내역이 확인되는데, 음영 색으로 표현
...
약 3분 간 전체적으로 30초마다 표 하나씩, 더 자세히 말하자면 각 계정별로 1분마다 표 하나씩 만든 결과

![image](https://user-images.githubusercontent.com/53200166/135217977-580fc1fa-644c-4439-a186-9dedf4139645.png)

위와 같이 30초 마다 버전 기록이 하나씩 툴페인에 생성되는 것을 볼 수 있으며, 각 계정별 편집 내역은 텍스트 단위로 이루어진다는 것을 알 수 있었음